...
import type { DisplayItem, SingleResult, SubagentDetails, UsageStats } from "./schema.ts";
...
export function truncateParallelOutput(output: string): string {
	...
	// Build prefix by code point so truncation never leaves a partial UTF-8 sequence.
	let truncated = "";
	let truncatedBytes = 0;
	for (const character of output) {
		const characterBytes = Buffer.byteLength(character, "utf8");
		if (truncatedBytes + characterBytes > PER_TASK_OUTPUT_CAP) break;
		truncated += character;
...
export function renderResult(result: AgentToolResult<SubagentDetails> | any, { expanded }: { expanded: boolean }, theme: any, _context?: any): any {
